...
export const SafetyStorage = ({ data }: { data: SafetyStorageType[] }) => {
  ...
  const getLastSafetyStorage = (
    pastSafetyStorage: { month: string; value: number }[]
  ) => {
    const date = new Date();
    const month = date.getMonth() == 0 ? 12 : date.getMonth();
    const year = month == 12 ? date.getFullYear() - 1 : date.getFullYear();

    return pastSafetyStorage.filter(
      (safetyStorage) => safetyStorage.month == year + "/" + month
    )[0].value;
  };
  ...
  const getSafetyStoragePercentage = (
    ...
    return (
      currentStorage /
      (((lastSafetyStorage + pastSalesSum) * 2) / (pastSales.length + 1))
    );
  };
      ...
      <div className="w-full h-full flex justify-start place-items-center space-x-4 overflow-y-scroll pl-7">
        {data.map((safetyStorage) => {
          ...
          if (filter === "all") return safetyStorageModal;
            ...
            if (
              safetyStoragePercentage <= 1.2 &&
              safetyStoragePercentage >= 0.8
            )
